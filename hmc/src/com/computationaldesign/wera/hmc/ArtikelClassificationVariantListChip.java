...
public class ArtikelClassificationVariantListChip extends GenericItemListChip
{
	private static final String JSP_URI = "/ext/wera/artikelClassificationVariantListChip.jsp";

	private Collection attributeNames;
	private Collection attributeNamesAssignedOnly;
	
	public ArtikelClassificationVariantListChip(DisplayState displayState, Chip parent, ComposedType type)
	{
...
*/	
	...
	{
		return new ArtikelClassificationVariantListEntryChip(getDisplayState(), this, item);								
	}
 	...
 	public Collection getClassificationAttributeNames(  )
	{
		if( attributeNames == null && !getListEntries().isEmpty() )
		{
			...
			final ArtikelClassificationVariantListEntryChip entryChip = (ArtikelClassificationVariantListEntryChip) getListEntries().get(0);
 	...
 	public Collection getClassificationAttributeAssignedOnlyNames(  )
	{
		if( attributeNamesAssignedOnly == null && !getListEntries().isEmpty() )
		{
			attributeNamesAssignedOnly = new ArrayList();
			
			final ArtikelClassificationVariantListEntryChip entryChip = (ArtikelClassificationVariantListEntryChip) getListEntries().get(0);
			
			for( final Iterator categoryIter = entryChip.getClassificationMapAssignedOnly().keySet().iterator(); categoryIter.hasNext(); )
			{
				for( final Iterator caIter = ((Collection) entryChip.getClassificationMapAssignedOnly().get(categoryIter.next())).iterator(); caIter.hasNext(); )
				{
					ClassificationAttributeEntry classificationAttributeEntry = ((ClassificationAttributeEntry) caIter.next());
					...
					EnumerationValue visibility = classificationAttributeEntry.getVisibility();
						...
						attributeNamesAssignedOnly.add( stringIcon + stringName  + stringUnit ); 
		...
		return attributeNamesAssignedOnly;
	}	
