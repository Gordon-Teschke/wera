...
public class ArtikelClassificationVariantListEntryChip extends EditableItemListEntryChip
{
	private static final String JSP_URI = "/ext/wera/artikelClassificationVariantListEntryChip.jsp";
	...
	public ArtikelClassificationVariantListEntryChip( DisplayState displayState, ItemListChip parent, Item item )
	{
	...
	{
		//System.out.println("ArtikelClassificationVariantListEntryChip.isChanged()");
		// normal attributes
		if( super.isChanged() )
		{
			return true;
		...
		for( final Iterator iter = getAllClassificationAttributeAssignedOnlyEntries().iterator(); iter.hasNext(); )
		{
			if( ((ClassificationAttributeEntry) iter.next()).isChanged() )
			{
				return true;
	...
	protected void refreshEntry()
	{
		...
		getClassificationMapAssignedOnly().clear();
	...
	public void save()
	{
		System.out.println ("+++++++++++++++ ArtikelClassificationVariantListEntryChip in Save (START)");
		// normal attributes
		super.save();
		// classification attributes
		for( final Iterator iter = getAllClassificationAttributeEntries().iterator(); iter.hasNext(); )
		...
		System.out.println ("+++++++++++++++ ArtikelClassificationVariantListEntryChip in Save (ENDE)");
	}
...
*/
	...
	public Collection getAllClassificationAttributeAssignedOnlyEntries()
	{
		Collection result = new ArrayList();
		
		for( final Iterator categoryIter = getClassificationMapAssignedOnly().keySet().iterator(); categoryIter.hasNext(); )
		{
			for( final Iterator entryIter = ((Collection) getClassificationMapAssignedOnly().get(categoryIter.next())).iterator(); entryIter.hasNext(); )
			{
